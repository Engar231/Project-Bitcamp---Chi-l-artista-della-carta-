...
def artista_mtg(img):
    ...
    match = difflib.get_close_matches(nickname, artisti_magic, n=1, cutoff=0.56)
...
def artista_digimon(img):
    ...
    match = difflib.get_close_matches(nickname, artisti_digi, n=1, cutoff=0.55)
    return match[0] if match else nickname
...
def artista_onepiece(img):
    ...
    roi_zoom = cv2.resize(roi, None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_CUBIC)
    nickname = ocr_generico(roi_zoom)
    match = difflib.get_close_matches(nickname, artisti_one, n=1, cutoff=0.55)
